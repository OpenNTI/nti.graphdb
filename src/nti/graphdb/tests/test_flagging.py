...
from nti.graphdb.flagging import add_flagged_relationship
from nti.graphdb.flagging import remove_flagged_relationship
...
class TestFlagging(GraphDBTestCase):
	...
	def test_flagging(self):
		...
		oid = get_oid(note)
		m = add_flagged_relationship(self.db, user.username, oid)
		assert_that(m, is_not(none()))

		rels = self.db.match(user, note, Flagged())
		assert_that(rels, has_length(1))
		
		remove_flagged_relationship(self.db, user.username, oid)
